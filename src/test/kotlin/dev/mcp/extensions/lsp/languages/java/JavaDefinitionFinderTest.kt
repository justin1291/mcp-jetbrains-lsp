...
class JavaDefinitionFinderTest : JavaBaseTest() {
    ...
    private lateinit var finder: JavaDefinitionFinder
    
    @BeforeEach
    override fun setUp() {
        super.setUp()
        finder = JavaDefinitionFinder()
    }
    ...
    fun testQualifiedNameSearch() {
        ApplicationManager.getApplication().runReadAction {
            ...
            if (definitions.isNotEmpty()) {
                ...
                assertEquals(1.0f, addUserMethod?.confidence ?: 0f, 
        ...
        ApplicationManager.getApplication().runReadAction {
            ...
            if (classPosition >= 0) {
                ...
                if (definitions.isNotEmpty()) {
                    ...
                    assertTrue(classDef.confidence >= 0.9f, 
    ...
    fun testReferenceResolution() {
        ApplicationManager.getApplication().runReadAction {
            ...
            if (userReferencePosition >= 0) {
                val definitions = finder.findDefinitionByPosition(psiFile, userReferencePosition + 2) // Position on "User"
                
                if (definitions.isNotEmpty()) {
                    val userClassDef = definitions.find { it.type == "class" && it.name == "User" }
                    if (userClassDef != null) {
                        assertEquals(1.0f, userClassDef.confidence,
                            "Reference resolution should have highest confidence")
                        assertTrue(userClassDef.filePath.contains("User.java"),
    ...
    fun testCreateLocationMethodSignature() {
        ApplicationManager.getApplication().runReadAction {
            ...
            if (definitions.isNotEmpty()) {
                ...
                assertTrue(toStringDef.signature?.contains("toString") ?: false,
