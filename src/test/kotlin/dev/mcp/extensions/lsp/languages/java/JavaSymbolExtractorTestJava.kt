...
class JavaSymbolExtractorTestJava : JavaBaseTest() {
    ...
    fun testExtractSymbolsFromUser() {
        ApplicationManager.getApplication().runReadAction {
            ...
            assertTrue(
                classes.any { it.name == "BaseEntity" },
    ...
    fun testExtractSymbolsFromApiController() {
        ApplicationManager.getApplication().runReadAction {
            ...
            val constants = symbols.filter { it.kind.value == "constant" }
            assertTrue(
                constants.any { it.name == "STATUS_SUCCESS" },
                "Should find STATUS_SUCCESS constant"
            )
            assertTrue(
                constants.any { it.name == "STATUS_ERROR" },
    ...
    fun testHierarchicalExtraction() {
        ApplicationManager.getApplication().runReadAction {
            ...
            assertTrue(
                nestedTypes.any { it.name == "UserSession" },
    ...
    fun testSymbolTypeFiltering() {
        ApplicationManager.getApplication().runReadAction {
            ...
            assertTrue(
                methodSymbols.all { it.kind.value == "method" },
                "Should only have methods"
            )
            assertTrue(
                methodSymbols.any { it.name == "addUser" },
            ...
            assertTrue(
                fieldSymbols.none { it.name == "DEFAULT_ROLE" },
            ...
            assertTrue(
                constructorSymbols.all { it.kind.value == "constructor" },
    ...
    fun testSemanticFiltering() {
        ApplicationManager.getApplication().runReadAction {
            ...
            val allSymbols = extractor.extractSymbolsFlat(
                psiFile, GetSymbolsArgs(
                    filePath = userServicePath,
                    hierarchical = false
                )
    ...
    fun testModifiersAndMetadata() {
        ApplicationManager.getApplication().runReadAction {
            ...
            assertTrue(
                oldDefaultRole.documentation?.isPresent == true,
            ...
            assertTrue(
                toStringMethod.documentation?.isPresent == true,
            ...
            assertEquals(
                Visibility.PROTECTED, oldDefaultRole.visibility,
                "OLD_DEFAULT_ROLE should be protected"
            )
            assertEquals(
                Visibility.PROTECTED, validateMethod.visibility,
            ...
            assertEquals(
                Visibility.PRIVATE, updateTimestamp.visibility,
            ...
            assertTrue(
                oldDefaultRole.decorators.any { it.name == "Deprecated" },
            ...
            assertTrue(
                toStringMethod.decorators.any { it.name == "Override" },
            ...
            assertTrue(
                validateMethod.decorators.any { it.name == "Deprecated" },
                "validate decorators should include @Deprecated"
            )
            assertTrue(
                validateMethod.decorators.any { it.name == "SuppressWarnings" },
    ...
    fun testMethodParametersAndReturnTypes() {
        ApplicationManager.getApplication().runReadAction {
            ...
            assertTrue(
                isValidUser.modifiers.contains("public"),
