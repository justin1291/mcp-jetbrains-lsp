...
class PythonReferenceFinder : PythonBaseHandler(), ReferenceFinder {
    ...
    private fun generatePythonSpecificInsights(
        references: List<ReferenceInfo>,
        element: PsiElement,
    ...
    private fun isInDeprecatedContext(element: PsiElement): Boolean {
        ...
        return extractDecorators(containingFunction ?: return false).any {
            it.contains(
                "deprecated",
                ignoreCase = true
            )
    ...
    private fun getPythonAccessModifier(
        element: PsiElement,
        @Suppress("UNUSED_PARAMETER") target: PsiElement
    ): String? {
        val parent = element.parent
        return when {
    ...
    private fun getPythonDataFlowContext(
        element: PsiElement,
        @Suppress("UNUSED_PARAMETER") target: PsiElement
    ): String? {
        val parent = element.parent
